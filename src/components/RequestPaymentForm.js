...
const RequestPaymentForm = ({ onClose }) => {
  ...
  const handleSubmitDetails = (e) => {
    ...
    // Generate payment link for the frontend payment page
    // Use the correct URL for the PaymentForm page (e.g., /pay) and encode the data as a URL parameter
    const baseUrl = window.location.origin;
    const encodedData = encodeURIComponent(JSON.stringify(requestData));
    const paymentLinkUrl = `${baseUrl}/pay?data=${encodedData}`;
    
    // Set the payment link in state
    setPaymentLink(paymentLinkUrl);
    
    // Log for debugging
    console.log('Generated payment link:', paymentLinkUrl);
  ...
  const handleSendEmail = async () => {
    try {
      ...
      // Create a parsed version of the request data from the payment link
      const paymentData = {
        sender: {
          wallet: publicKey.toString(),
          name: publicKey ? `User (${publicKey.toString().slice(0, 6)}...)` : 'Sienna User'
        },
        recipient: {
          name: recipient.name,
          email: recipient.email,
      ...
      const emailData = {
        ...
        dueDate: dueDate || undefined,
        paymentLink,
        senderName: publicKey ? `Sienna User (${publicKey.toString().slice(0, 6)}...)` : 'Sienna User',
        senderWallet: publicKey?.toString(),
        paymentData: paymentData // Include full payment data for reference
      };
      
      console.log('Sending email data:', emailData);
      
      // Call the backend API using environment variable
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/email/payment-request`, {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-5">
      <div>
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
          ...
          {!emailSent && !sendingEmail && !emailError && (
            <button
